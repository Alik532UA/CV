...
describe("CSP — the first-frame script's hash is registered", () => {
	...
	it("хеш обчислюється з app.html, а не вписаний рядком", () => {
		/*
		 * Перевірка НЕ на близькість рядків.
		 *
		 * Тут стояло `createHash[\s\S]{0,200}?app\.html|app\.html[\s\S]{0,300}?createHash`
		 * — тобто «обидві згадки в межах 200/300 символів». 2026-08-23 у конфіг
		 * додали докблок про нормалізацію CRLF, і перевірка почервоніла, хоча код
		 * став ПРАВИЛЬНІШИМ: між згадками просто побільшало тексту. Гейт, який
		 * червоніє від коментаря, — це гейт, який вимикають.
		 *
		 * Тепер перевіряється сам факт: конфіг читає `src/app.html` І хешує.
		 * А те, що обчислений хеш справді той, який приймає браузер, доводить
		 * `src/csp-hash.test.ts` — твердження строго сильніше за будь-яку
		 * текстову близькість.
		 */
		expect(
			SVELTE_CONFIG,
			"конфіг не читає src/app.html — хеш нізвідки брати"
		).toMatch(/readFileSync\([^)]*app\.html/);
		expect(
			SVELTE_CONFIG,
			"конфіг не хешує — хеш або вписаний рядком, або відсутній"
		).toMatch(/createHash\(["']sha256["']\)/);
...
describe("hold-scroll canon — автоматичний рух вимикається настройкою", () => {
	...
	it("механіка вимикається ще й перемикачем, і обидва читаються при кожній спробі", () => {
		// Прапорець без чекбокса — мертвий код, який читається як зроблена
		// робота (HOLD-SCROLL § 1.1). Тут перевіряється половина, доступна
		// джерелам: клас бере стан ФУНКЦІЄЮ, тож перемикання посеред сесії
		// доходить. Другу половину — що чекбокс справді намальований —
		// тримає e2e.
		const src = read(HOLD);
		// `[\s\S]*?`, а не `[^)]*`: перший же `)` тут закриває `() => HoldGeometry`
		// у сусідньому параметрі, і клас межі не має.
		expect(src, "конструктор мусить брати другим аргументом читача стану").toMatch(
			/constructor\([\s\S]*?enabled:\s*\(\)\s*=>\s*boolean/
		);
		expect(
			src,
			"стан читається викликом на кожну спробу; збережене поле пережило б перемикання"
		).toMatch(/this\.enabled\(\)/);
	});

	it("чекбокс показується за active, а не за mode", () => {
		// § 1.3: `mode` лишається custom/minimap, поки малює НАТИВНА смуга —
		// на сенсорному екрані й у вікні вужчому за 1100 px. Умова на `mode`
