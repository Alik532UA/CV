...
describe("числа баз у документах — під гейтом (PIT-NUMBER-UNDER-GATE)", () => {
	...
	function claimedNumbers(line: string): number[] {
		const cleaned = line
			...
			.replace(/§\s*[\d.]+/g, " ")
			// Номер критерію WCAG — така сама адреса розділу, як `§`, і так само
			// не є заміром. Без цього `WCAG 1.4.10` розбиралося на «1.4» (як
			// дріб) і залишок «10», який виглядав лічильником.
			.replace(/\b(?:WCAG|SC)\s*\d+(?:\.\d+)*/gi, " ")
			.replace(/-?v\d+(?:\.\d+)?/gi, " ")
			.replace(/\d+[.,]\d+\s*:\s*\d+/g, " ")
	...
	it("перевірка жива: бази читаються, ідіоми знімаються, рядки знаходяться", () => {
		...
		expect(claimedNumbers("WCAG 1.4.10, критерій SC 2.1.4")).toEqual([]);
