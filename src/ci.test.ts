 ...
 * дереві лежать із `\n`, і розбір ніколи не бачив нуля. Коли ці рядки писалися,
 * трималося це на випадковості — форму закінчень не фіксувало ніщо. Відтоді
 * зʼявився `.gitattributes` із `* text=auto eol=lf`, тобто свіжий `git clone`
 * на Windows із `core.autocrlf=true` (типовий вибір інсталятора git) більше не
 * віддає `\r\n`, і головний шлях до сліпого розбору закритий у корені.
 *
 * Нормалізація тут лишається, і не з інерції. `.gitattributes` керує тим, що
 * git КЛАДЕ в робоче дерево, і мовчить про все інше: файл, дописаний редактором
 * із власним налаштуванням, згенерований скрипт, вміст, вставлений із буфера.
 * Два незалежні захисти від одного класу — рівно та ціна, яку цей клас уже
 * стягнув із сусідніх проєктів пакета (`teatralo4ka.odesa.ua`, `MindStep`):
 ...
 * `CI-ACTION-RUNTIME`, MEDIUM; `GATE-CI-PIPELINE`).
 *
 * ЩО САМЕ ЛОВИТЬ ЦЕЙ ЛЕДЕР, А ЩО НІ. Рантайм дії лежить у полі `runs.using`
 * її `action.yml` і з номера релізу НЕ виводиться: канон заміряв 2026-08-23
 * дві дії, які на найновішому тоді мажорі стояли на `node20`, тобто «підняти
 * на наступний мажор» попередження про застарілий рантайм із них не знімало.
 * Прочитати це поле можна лише запитом до репозиторію самої дії, а гейт, який
 * лізе в мережу, ламається без неї — і ламається в CI, де мережа є, тихіше за
 * все. Тому тут НЕ перевіряється значення `runs.using`.
 *
 * Перевіряється інше й важливіше: щоб перелік дій у workflow і перелік
 * заміряних дій не розходилися мовчки. Правило каже звіряти рантайм ПРИ
 * КОЖНОМУ підйомі; доти це трималося на тому, що хтось про це пам'ятатиме, і
 * єдиним слідом був абзац у `PROJECT-CONTEXT.md`. Тепер підйом мажора робить
 * прогін червоним і друкує команду, якою міряють.
 *
 * ЧОМУ ЗНАЧЕННЯ ЗАРАЗ «не заміряно», А НЕ ЧИСЛО. Записати сюди рантайм із
 * пам'яті заборонено тим самим правилом, що й решту чисел
 * (AI-AGENT-PITFALLS-v9 § 5.5.1): це виглядало б заміром, читалося б заміром і
 * старіло б тихо. Шість «не заміряно» — це чесний стан боргу, до того ж
 * лічильний: досі він був прозою в одному рядку документа.
 *
 * ЯК ЗАМІРЯТИ (по одній дії, результат вписати замість «не заміряно» разом із
 * датою):
 *
 *   gh api repos/<owner>/<action>/contents/action.yml --jq '.content' \
 *     | base64 -d | grep "using:"
 *
 * Дія іншого автора «за компанію» не піднімається, і дія на composite Node не
 * запускає взагалі — попередження тоді дає ВКЛАДЕНА дія, якої у workflow
 * немає (§ 1.9). Обидва випадки описуються тут же, у значенні запису.
 */
const ACTION_RUNTIME: Record<string, string> = {
	'actions/checkout@v7': 'не заміряно',
	'actions/setup-node@v7': 'не заміряно',
...
describe('рантайм дій CI звіряється при підйомі (CI-ACTION-RUNTIME)', () => {
	/** `owner/action@major` кожного `uses:` у workflow. */
	const used = [
		...new Set(
			[...all.matchAll(/uses:\s*([\w.-]+\/[\w.-]+)@(v\d+)/g)].map((m) => `${m[1]}@${m[2]}`)
		)
	].sort();
	...
	it('розбір живий: дії у workflow знайдено', () => {
		expect(
			used.length,
			'у workflow не знайдено жодного `uses:` — розбір читає не те, і ледер нижче ' +
				'доводив би щось про себе самого'
		).toBeGreaterThan(3);
	});

	it('кожна дія з workflow є в ледері', () => {
		const missing = used.filter((a) => !(a in ACTION_RUNTIME));
		expect(
			missing,
			'мажор дії піднято, а рантайм при цьому не звірено. Номер релізу про\n' +
				'`runs.using` не каже нічого, тож підйом сам по собі попередження не знімає.\n' +
				'Заміряти й вписати в ACTION_RUNTIME:\n' +
				missing
					.map(
						(a) =>
							`  gh api repos/${a.split('@')[0]}/contents/action.yml --jq '.content' | base64 -d | grep "using:"`
					)
					.join('\n')
		).toEqual([]);
	});

	it('у ледері немає запису про дію, якої у workflow вже немає', () => {
		const stale = Object.keys(ACTION_RUNTIME).filter((a) => !used.includes(a));
		expect(
			stale,
			'запис пережив саму дію: наступний прочитає його як стан пайплайну,\n' +
				`а це історія: ${stale.join(', ')}`
		).toEqual([]);
	});

	it('кожен запис ледера має або замір, або названу причину його відсутності', () => {
		const empty = Object.entries(ACTION_RUNTIME)
			.filter(([, note]) => note.trim().length < 5)
			.map(([action]) => action);
		expect(
			empty,
			`запис без значення читається як «заміряно й усе гаразд»: ${empty.join(', ')}`
		).toEqual([]);
	});
});
