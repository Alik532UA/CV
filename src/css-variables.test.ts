 ...
 * across 120 references. Here it found six on arrival, in two flavours:
 *
 *  - `--error-bg` / `--error-text` / `--error-border` in `ErrorFallback.svelte`,
 *    used with NO fallback. That component is what `<svelte:boundary>` shows
 *    when a section of the main page throws, and it is wired at five section
 *    boundaries. The one panel whose whole job is to look unlike the page
 *    around it had no background and no border;
 *  - `--score-high` / `--score-mid` / `--score-low` in the AI panels, used with
 *    fallbacks repeated across five places — the same three literals in both
 *    themes, including a cyan that sits on the light theme at about 1.3:1.
 *
 * All six are now declared in `src/app.css` for both themes.
 *
...
const CROSS_COMPONENT: Record<string, { declaredIn: string; why: string }> = {
	"--score-color": {
		declaredIn: "src/lib/components/ui/AiMatchModal.svelte",
		why: "the ring sets it inline from the score, the ring's own rules read it"
	}
};
...
describe("CSS variables", () => {
	...
	it("finds sources, declarations and references — the check is alive", () => {
		...
		expect(declaredGlobally.size).toBeGreaterThan(20);
