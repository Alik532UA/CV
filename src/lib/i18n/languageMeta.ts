...
/**
 * What the collapsed switcher button shows beside the flag. A code with a region
 * subtag reports just the region ("en-us" → "US"), so the badge stays as narrow
 * as the plain codes in a header that is already tight on a phone — and the flag
 * next to it is what says which English this is.
 */
export function languageBadge(code: Language): string {
...
export const LANGUAGE_META = [
    ...
    // optimisation/optimization, licence/license). "en" is the British text and
    // the site default; the label says so, since "English" on its own next to a
    // second English entry tells the visitor nothing.
