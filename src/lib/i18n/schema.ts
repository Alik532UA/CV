...
export interface Translations {
	...
	pdf_modal: {
		...
		light: string;
		/**
		 * `alt` прев'ю резюме. `{variant}` — підпис версії (`dark` / `light`).
		 *
		 * Стояв літералом англійською в `cvDownloads.ts` — тобто в `.ts`, куди
		 * сканер англійських атрибутів не заглядає взагалі: він читає розмітку,
		 * а там значення приходить виразом `alt={file.alt}`. Читалка вимовляла
		 * «Dark Theme CV Preview» відвідувачеві японської сторінки.
		 */
		previewAlt: string;
	};
	/**
	...
	ui: {
		...
		pdfVersion: string;
		/**
		 * Підказка на мові, позначеній `*`: переклад машинний і носієм не
		 * вичитаний.
		 *
		 * Стояла літералом англійською в `HeaderSection.svelte` і була невидима
		 * для сканера атрибутів, бо жила у ВИРАЗІ (`умова ? "…" : undefined`), а
		 * не в `title="…"`. Тобто попередження про машинний переклад саме було
		 * неперекладене — у меню, яке відкривають рівно для того, щоб піти на
		 * іншу мову.
		 */
		machineTranslated: string;
		/**
		 * Доступне ім'я службового знака версії. `{version}` — номер збірки: з
		 * ним звіт логів прив'язується до конкретного релізу.
		 *
		 * Знак видимий у dev завжди, а в проді — після `?debug=1` або серії
		 * натискань `V`. Шлях через адресу існує саме для телефона, тобто це не
		 * суто авторський інструмент.
		 */
		copyReport: string;
	};
	common: {
