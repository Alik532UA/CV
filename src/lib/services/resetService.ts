 ...
 * `cv-svelte_` prefix, which is exactly why the reset goes through it rather than
 ...
 * single source as everything else: `STORAGE_PREFIX` and `base`.
 *
 ...
 */
const CONFIRM_TEXT =
	"This clears all local data for this site: preferences, chat history, beta marks. Continue?";
 ...
 * (`https://host/CV/`) while `base` is a path (`/CV`), so a direct `startsWith`
 * would never match.
 */
 ...
 * not delete the entry but duplicates it. `base` IS that path: `/CV` in
