...
/**
 * Стеля початкового JS у КБ gzip. Виміряно цим самим скриптом: 242 (2026-08-16),
 * 244 (2026-08-19 — маршрут `/beta-test-checklists/` додав два кілобайти в
 * маніфест роутера; сама сторінка окремим чанком і на першу не їде).
 * Запас навмисно малий — бюджет має ловити приріст, а не пропускати його.
 * Число рухається ЛИШЕ вниз.
 */
const BUDGET_KB = 250;

/** Найбільший окремий чанк. Ловить те, чого не видно в сумі: одна залежність,
 *  яка тихо подвоїлася, поки решта худла. Виміряно 2026-08-16: 131 КБ —
 *  це словники 42 мов, які їдуть одним чанком. */
const LARGEST_CHUNK_KB = 135;
