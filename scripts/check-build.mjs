...
// ---------------------------------------------------------------------------
/*
 * SEC-CSP-SPREAD-HANDLER (SECURITY-v9 § 6.3.2, HIGH) — інлайнові обробники,
 * яких ніхто не писав.
 *
 ...
 * розмітці. І ХЕШІ ТУТ НЕ РЯТУЮТЬ У ПРИНЦИПІ — сам браузер каже це прямо:
 * «Note that hashes do not apply to event handlers». Тобто механізм, яким
 * покритий інлайн-скрипт першого кадру (§ 6.3, `csp-hash.test.ts`), на цей
 * клас не поширюється взагалі.
 *
 * ЧОМУ ЦЬОГО НЕ ВИДНО В ДЖЕРЕЛАХ. Таких атрибутів ніхто не пише — їх додає
 * компілятор Svelte 5 під час SSR. Механізм відтворення подій
 * (`onload="this.__e=event"`) потрібен на випадок, коли зображення
 * завантажилося ДО гідрації й подія інакше загубилася б. Гачок вставляється в
 * елемент, чиї атрибути задані РОЗГОРТАННЯМ (`{...obj}`): що лежить в
 * обʼєкті, компілятор не знає, тож припускає найгірше.
 *
 * Заміряно в сусідньому проєкті пакета 2026-08-26: тринадцять `<img>`,
 * переведених на розгортання, дали 15 порушень CSP на головній сторінці — при
 * чистому `svelte-check` і 715 зелених юніт-перевірках. Тут ризик прямий:
 * `PdfModal.svelte` і `ProjectsSection.svelte` малюють `<img>` із width/height
 * із конфігу, і `{...imageSize(file)}` — рівно та правка, яку зробили б
 * «щоб не повторювати два атрибути».
 *
 * ПЕРЕЛІК АТРИБУТІВ, А НЕ `/\bon[a-z]+=/`: друге ловить і `only=`, і
 * `data-once="…"`, і кожен майбутній атрибут зі складом «on» усередині.
 */
...
const HANDLER_ATTR = new RegExp(`\\s(?:${INLINE_HANDLERS.join("|")})\\s*=\\s*["'][^"']*["']`, "gi");

for (const file of files) {
	const html = readFileSync(file, "utf8");
	for (const m of html.matchAll(HANDLER_ATTR)) {
		fail(
			`${file}: інлайновий обробник${m[0].trim().split("=")[0] ? ` ${m[0].trim().split("=")[0]}` : ""} ` +
				`у зібраному HTML — CSP його заблокує (хеші на обробники не діють). ` +
				`Причина зазвичай у розгортанні атрибутів {...obj} на <img>/<iframe>/<video>`
		);
	}
}
